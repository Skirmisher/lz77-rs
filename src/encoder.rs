use buffer::{CombinedBuffer, RingBuffer, SizedBuffer};
	...
	S: Searcher + Default,
{
	...
	fn write_output_buffer(&mut self) -> io::Result<()> {
		if self.output_buffer.len() > 0 {
			let code = CodeWord::new_with_data(
				16 - self.options.window_size,
				0u16,
	...
	fn write_to_inner(&mut self, token: Lz77EncoderToken, next: u8) -> io::Result<()> {
		...
		match token {
			Lz77EncoderToken::Match(dist, len) => {
				...
				let code = match CodeWord::new_with_data(
					16 - self.options.window_size,
					dist as u16,
					len as u16,
				) {
					...
					Err(e) => {
						panic!(
							"Somebody screwed up with the CodeWord size ({}, {}), Err = {}",
							dist, len, e
						);
					}
				};
	...
	fn write(&mut self, full_buf: &[u8]) -> io::Result<usize> {
		...
		while let Some(n) = self.fill_forward_buffer(buf) {
			let search_result = {
				...
				self.searcher.find_longest_match(
					&search_buf,
					&self.unmatched_data[..(self.forward_search_size - 1)],
				)
			};
	...
	fn flush(&mut self) -> io::Result<()> {
		while self.unmatched_data.len() != 0 {
			let search_result = {
				...
				self.searcher
					.find_longest_match(&search_buf, &self.unmatched_data[..(search_size)])
			};
